...
uses 
  ...
  System.Classes,
  Web.HTTPApp,
  MVCFramework, IdBaseComponent, IdComponent, IdIOHandler, IdIOHandlerSocket,
...
begin
  ...
  FMVC.OnWebContextDestroy(
   ...
   '/swagger',
   TPath.Combine(ExtractFilePath(GetModuleName(HInstance)), 'swagger-ui'))
  );

  var swagger : TMVCSwaggerInfo;
  swagger.Title := 'Jamensoft Comunicacion Emisor-Receptor';
  swagger.Version := 'v1';
  swagger.Description := 'EndPoint Receptor Jamensoft.';
  FMVC.AddMiddleware(TMVCSwaggerMiddleware.Create(FMVC, swagger,'/api/swagger.json'));
end;
